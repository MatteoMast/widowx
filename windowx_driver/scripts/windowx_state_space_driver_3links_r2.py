...
class WindowxNode(ArbotiX):
    ...
    def __init__(self):
        ...
        print"\nArbotix initialization for r2, wait 10 seconds..."
        ArbotiX.__init__(self, port="/dev/ttyUSB1")
        for x in xrange(1,21):
            time.sleep(0.5)
            print(str(x*0.5) + "/10s for r2")
            if rospy.is_shutdown():
                break
        print"Done."

        #Set initial torque limits
        ...
        self.pos_pub = rospy.Publisher('/windowx_3links_r2/joints_poses', Float32MultiArray, queue_size=1)
        self.vel_pub = rospy.Publisher('/windowx_3links_r2/joints_vels', Float32MultiArray, queue_size=1)
        self.pub_rate = rospy.Rate(150)

        #ROS listener for control torues
        self.torque_sub = rospy.Subscriber('windowx_3links_r2/torques', Float32MultiArray, self._torque_callback, queue_size=1)
        self.gripper_sub = rospy.Subscriber('windowx_3links_r2/gripper', Bool, self._gripper_callback, queue_size=1)

        print"\nWindowx_3link_r2 node created, whaiting for messages in:"
        print"      windowx_3links_r2/torque"
        print"Publishing joints' positions and velocities in:"
        print"      /windowx_3links_r2/joints_poses"
        print"      /windowx_3links_r2/joints_vels"
        #Start publisher
        self.publish()
    ...
    def _torque_callback(self, msg):
        ...
        if goal_torque_steps[0] == int(max1) or goal_torque_steps[1] == int(max2) or goal_torque_steps[2] == int(max3):
            print("\nWARNING, R2 MAX TORQUE LIMIT REACHED FOR ID: ")
    ...
    def publish(self):
        ...
        while not rospy.is_shutdown():
            ...
            if not -1 in present_vels and not -1 in present_positions and not 65535 in present_vels and not 65535 in present_positions:
...
if __name__ == '__main__':
    #Iitialize the node
    rospy.init_node('windowx_3links_driver_r2')
